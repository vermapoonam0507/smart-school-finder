...
import { MapPin, BookOpen, Users, Heart, Building, Award, Sun, CheckCircle } from 'lucide-react';
...
useEffect(() => {
    ...
    const handleApplyNow = () => {
        ...
        if (!currentUser) {
            ...
            // Let the ProtectedRoute handle the redirect. It will save this path.
            navigate(`/apply/${school._id}`);
        } else if (currentUser.userType !== 'parent' && currentUser.userType !== 'student') {
            toast.error("Only Parents or Students are allowed to apply.");
        } else {
            // If user is a parent/student, navigate to the application form
            navigate(`/apply/${school._id}`);
        }
    };
    ...
    if (!school) {
        return (
            <div className="flex flex-col justify-center items-center h-screen">
                ...
                <button onClick={() => navigate('/schools')} className="mt-4 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700">Back to Schools</button>
            </div>
        );
    }
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="bg-white shadow-lg rounded-lg p-6 mb-8 relative">
                    ...
                    <div className="mt-6 flex flex-wrap gap-3">
                        ...
                        <button onClick={handleApplyNow} className="border border-indigo-600 text-indigo-600 px-6 py-2 rounded-lg hover:bg-indigo-50 transition-colors">Apply Now</button>
                    </div>
                </div>
                ...
                <div className="bg-white shadow-lg rounded-lg p-6">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-3">Amenities & Activities</h2>
                        <div className="flex flex-wrap gap-2">
                           {school.predefinedAmenities?.map(amenity => (
                                <span key={amenity} className="bg-purple-100 text-purple-800 text-sm font-medium px-3 py-1 rounded-full flex items-center"><CheckCircle size={14} className="mr-1.5" />{amenity}</span>
                           ))}
                           {school.activities?.map(activity => (
                                <span key={activity} className="bg-teal-100 text-teal-800 text-sm font-medium px-3 py-1 rounded-full flex items-center"><CheckCircle size={14} className="mr-1.5" />{activity}</span>
                           ))}
                        </div>
                    </div>
                </div>
