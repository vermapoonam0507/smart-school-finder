...
const Header = ({ onNavigate, isMobileMenuOpen, setMobileMenuOpen, compareCount, currentUser, onLogout }) => {
  ...
  const handleRegisterClick = () => {
    if (currentUser && currentUser.role === 'school') {
      ...
      onNavigate('signup-school'); // Hamesha school signup page par bhejein
    }
  };
      ...
      <div className="hidden md:flex items-center space-x-4">
        {currentUser ? (
            ...
            <>
                ...
                <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors" onClick={handleRegisterClick}>Register Your School</button>
            </>
        )}
      </div>
        ...
        <div className="px-6 py-4 border-t">
          {currentUser ? (
            ...
            <>
              ...
              <button className="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700" onClick={() => { onNavigate('signup-school'); setMobileMenuOpen(false); }}>Register Your School</button>
            </>
          )}
        </div>
      </div>
