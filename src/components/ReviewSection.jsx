...
const ReviewSection = ({ schoolId }) => {
  ...
  const handleLikeReview = async (reviewId) => {
    ...
    if (currentUser.userType === 'school' || currentUser.userType === 'admin') {
      toast.error('Only students can like reviews');
      return;
    }
    ...
    if (review && review.likedBy && review.likedBy.includes(studentId)) {
      ...
      setReviews(prev => prev.map(review => 
        review._id === reviewId 
          ? { 
              ...
              likedBy: [...(review.likedBy || []), studentId]
            }
          : review
      ));
    } catch (error) {
      console.error('Error liking review:', error);
      const errorMessage = error.response?.data?.message || 'Failed to like review';
      toast.error(errorMessage);
    } finally {
                  ...
                  <div className="flex items-center space-x-4">
                    <button
                      ...
                      className={`flex items-center space-x-1 px-3 py-1 rounded-full text-sm transition-colors ${
                        review.likedBy && review.likedBy.includes(currentUser?.authId || currentUser?._id)
                    ...
                    >
                      <Heart 
                        size={16} 
                        className={review.likedBy && review.likedBy.includes(currentUser?.authId || currentUser?._id) ? 'fill-current' : ''}
                      />
